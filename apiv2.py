...
from haptic_output.haptic_output_v2 import HapticOutput
from localisation.localisationv2 import Localisation
...
def find_nearest_landmark(x, y):
    ...
    try:
        ...
        print("Converted landmark", (round_coord(
            nearest_landmark[0]), round_coord(nearest_landmark[1])))
        ...
        distance = distances["NEAR"] if shortest_distance <= 2 else distances[
            "MIDDLE"] if 5 >= shortest_distance > 2 else distances["FAR"]
...
def map_sequence(sequence):
    try:
        ...
        add_or_update_sequence_mapping(
            (x, y), sequence)
        play_entered_sequence(format_sequence_int(sequence))
...
def play_ack_sequence():
    try:
        ...
        haptic_output.play_ack_sequence()
...
def update():
    try:
        ...
        destination = (round_coord(
            destination[1]), round_coord(destination[0]))
