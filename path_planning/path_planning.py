...
class PathPlanner:
    ...
    def get_target_heading(self, node1, node2):
        ...
        print(required_movement_direction_x, required_movement_direction_y)
    ...
    def map_angle_to_direction(self, heading_change):
    ...
    def __translate_node(self, node):
        ...
        translated_y = ((self.map_height - node[0])*self.node_density)-y_origin
    ...
    def calculate_next_direction(self, start, end, heading, offset, print_map=False, print_path=False, hug_objects=False):
        try:
            ...
            start = tuple((self.map_height - start[0], start[1]))
            end = tuple((self.map_height - end[0], end[1]))
            ...
            neighbours = self.get_cell_radius(start[0], start[1])
            if end in neighbours or end == start:
                # User has arrived
            ...
            if path:
                ...
                if start != path[0]:
                    ...
                    required_heading = self.get_target_heading(start, path[0])
                else:
                    # else required heading is towards next step
                    required_heading = self.get_target_heading(path[0], path[1])
                ...
                turn_direction = self.map_angle_to_direction(heading_change)
